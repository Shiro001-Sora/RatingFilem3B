...
function createMovieCard(movie) {
    ...
    // Compute details path so links work both from root and from pages inside `html/`
    const detailsPath = (window.location.pathname.includes('/html/') || window.location.pathname.includes('html/'))
        ? 'details.html'
        : 'html/details.html';
    ...
    card.innerHTML = `
        ...
        <div class="movie-info">
            ...
            <a href="${detailsPath}?id=${movie.id}" class="view-details btn">View Details</a>
            </div>
        </div>
    `;
...
// Show Add Movie Button and Modal only for admin (use auth helper)
if (typeof isAdminLoggedIn === 'function' && isAdminLoggedIn()) {
    ...
    if (addBtn) {
        ...
        addBtn.onclick = () => { if (modal) modal.style.display = 'block'; };
    ...
    if (document.getElementById('add-movie-form')) {
        document.getElementById('add-movie-form').onsubmit = function(e) {
            ...
            form.reset();
            if (document.getElementById('movie-search')) {
                filterMovies(document.getElementById('movie-search').value);
            } else if (typeof loadAllMovies === 'function') {
                loadAllMovies();
            }
        };
    }
